...
namespace FYH.Cookbook.DAO.Concretes
{
    public class IngredientRepository : IIngredientRepository
    {
        ...
        public PagingResult<Ingredient> SearchIngredient(string keyword, IEnumerable<int?> ingredientIds, IEnumerable<int?> tagIds, IDictionary<string, SqlSortedEnum> orders, int page, int rows)
        {
            var sql = new StringBuilder(@"
select * from Ingredient i
where exists(
    select * from Recipe r, RecipeIngredientMapping rim
    where r.IsDeleted = 0 and rim.IngredientId = i.IngredientId and r.RecipeId = rim.RecipeId 
");
            ...
            return BaseRepository.ExecutePagingList<Ingredient>(sql.ToString(), orders, parameters, (page - 1) * rows, rows);
        }
    }
}
